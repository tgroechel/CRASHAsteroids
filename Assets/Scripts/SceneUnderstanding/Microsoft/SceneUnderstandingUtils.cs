...
namespace Microsoft.MixedReality.SceneUnderstanding.Samples.Unity
{
    ...
    public class SceneUnderstandingUtils : MonoBehaviour
    {
        ...
        public void AddMeshRenderingComponents(GameObject go, Mesh mesh, Material material, Color? color)
        {
            ...
            // Add Box collider to represent wall in physics space
            go.AddComponent<BoxCollider>();
            BoxCollider collider = go.GetComponent<BoxCollider>();
            Vector3 size = Vector3.zero;
            float minThickness = 0.1f;
            size.x = mesh.bounds.size.x < minThickness ? minThickness : mesh.bounds.size.x;
            size.y = mesh.bounds.size.y < minThickness ? minThickness : mesh.bounds.size.y;
            size.z = mesh.bounds.size.z < minThickness ? minThickness : mesh.bounds.size.z;
            collider.size = size;
        }
